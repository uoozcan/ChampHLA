...
**Table 3 — statistical summary (paired tests, held-out nested CV).** *b/c are discordant-locus counts for the exact McNemar test (b = ChampHLA correct and comparator not; c = the reverse).*
...
**Table 3 — sensitivity of the reported accuracy to the weighting scheme (overall correct-call rate).** *Same champion selection and, where applicable, override policy; only the tool weights or the override gate are changed.*
...
**Table 3 — override-gate audit (held-out overrides, benchmark configuration).** *Each triggered override is classified against truth as corrective (override right, champion wrong), harmful (override wrong, champion right), or neutral (both wrong).*

#!/bin/bash
#SBATCH --job-name=giab_rna_HG002
...
#SBATCH --mem=32G
#SBATCH --output=/scratch/project_2008084/hla_calibration/logs/giab_rna_%j.out
#SBATCH --error=/scratch/project_2008084/hla_calibration/logs/giab_rna_%j.err

# GIAB Ashkenazi-trio RNA (HG002/HG003/HG004) vs Stanford clinical SBT gold truth.
...
# After: python3 bin/run_1000g_benchmark.py --config conf/benchmark_giab_rna.yaml \
#          --output-dir analysis/giab_trio_benchmark/rna
set -uo pipefail   # NOT -e: a single bad download must skip its line, not abort the job
module load nextflow
module load seqtk

BASE=/scratch/project_2008084/hla_calibration/giab_trio/rna
STAGE=$BASE/stage; RESULTS=$BASE/results
INDEX=/scratch/project_2008084/pihla-publish/analysis/giab_trio_benchmark/index/rna_single_HG002.tsv
WORK=/scratch/project_2008084/hla_calibration/work/giab_rna_HG002
SUBN=30000000
...
nextflow run main.nf \
  ...
  -resume -name giab_rna_${SLURM_JOB_ID} \
  -w "$WORK" \
  --input "$STAGE" \
  --input_type fastq \
